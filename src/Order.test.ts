...
test("Should calculate the area of circle", function () {
  ...
  const order = new Order(cpfNumber,purchaseDescription,price,quantity,discount);
  expect(order.orderPrice()).toBe(142.5);

  const cpftest = new Order(cpfNumber,purchaseDescription,price,quantity,discount);
